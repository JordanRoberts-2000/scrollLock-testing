...
const CategoryImage = ({imageUrl, active, title, subtitle, blurImageUrl, index, imageRef, imageWrapperRef, imageFixed, transitioning, scrollUpRef}: Props) => {
    ...
    return (
        <div ref={imageWrapperRef} className="h-full w-full relative duration-700 overflow-hidden">
                <div className={`${imageFixed ? "fixed top-0 left-0 w-full aspect-[3/3.1] z-30 overflow-hidden" : 'relative h-full w-full'}`}>
                <div className="absolute top-0 w-full h-full bg-white/40 z-10"></div>
                <Image ref={imageRef} alt="placeholder" priority={index <= 1} fill src={imageUrl} className={`${powerSavingMode && '!scale-100'} object-cover transition-transform select-none scale-150 duration-75`} 
